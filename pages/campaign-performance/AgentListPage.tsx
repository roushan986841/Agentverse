...
import { CAMPAIGN_PERFORMANCE_AGENTS_LIST } from '../../constants.tsx';
import Breadcrumbs from '../../components/Breadcrumbs.tsx';

const DetailSection: React.FC<{ title: string; children: React.ReactNode, isCode?: boolean }> = ({ title, children, isCode = false }) => (
    <div className="mb-6">
        <h4 className="text-lg font-semibold text-[#002D72] mb-2">{title}</h4>
        {isCode ? (
            <pre className="bg-slate-100 p-4 rounded-md text-sm text-slate-700 whitespace-pre-wrap font-mono border border-slate-200">{children}</pre>
        ) : (
...
const AgentDetailModal: React.FC<{ agent: any | null, onClose: () => void }> = ({ agent, onClose }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col animate-scale-in"
...
const AgentTile: React.FC<{ agent: any, onDetailsClick: (agent: any) => void }> = ({ agent, onDetailsClick }) => {
    const statusClass = agent.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800';
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200/80 flex flex-col justify-between h-full transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            <div>
                <div className="flex items-start justify-between mb-4">
                    <div className="w-10 h-10 mr-4 text-[#002D72] flex-shrink-0">{agent.icon}</div>
                     <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${statusClass}`}>{agent.status}</span>
                </div>
                <h3 className="text-lg font-semibold text-slate-800 leading-tight mb-4">{agent.name}</h3>
            ...
            <div className="flex flex-col gap-2 mt-auto">
                <button
                    ...
                    className="w-full text-center bg-[#002D72] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#001E4D] transition-colors duration-300 text-sm"
                >
                    View Details
                </button>
            </div>
...
const AgentListPage = () => {
    ...
    return (
        <>
            <div className="max-w-7xl mx-auto p-6 sm:p-8 lg:p-10 animate-fade-in">
                <div>
                    <Breadcrumbs />
                    <header className="text-center mb-10">
                        <h1 className="text-4xl md:text-5xl font-extrabold text-[#002D72]">Active Campaign Performance Agents</h1>
                        <p className="mt-4 text-lg text-slate-600 max-w-3xl mx-auto">Manage your deployed campaign performance agents. View their configuration details or run a demo.</p>
                    </header>
                </div>
              
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {CAMPAIGN_PERFORMANCE_AGENTS_LIST.map(agent => (
              ...
              <div className="mt-12 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <a
                        ...
                        className="inline-flex items-center bg-slate-200 text-slate-700 font-bold py-3 px-6 rounded-lg hover:bg-slate-300 transition-colors duration-300 shadow-sm"
                    >
                        Setup Docs
                    </a>
                </div>
