        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-8 overflow-y-auto">
                    <DetailSection title="Set-up Details">{agent.setupDetails}</DetailSection>
                    <DetailSection title="Prompt Template" isCode>{agent.promptTemplate}</DetailSection>
                    <DetailSection title="Business Rules / Action Template" isCode>{agent.businessRules}</DetailSection>
                    <DetailSection title="Output Template">{agent.outputTemplate}</DetailSection>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 flex flex-col justify-between h-full transition-shadow hover:shadow-lg">
            ...
            <div className="flex flex-col gap-2 mt-auto">
                <button onClick={() => onDetailsClick(agent)} className="w-full text-center bg-[#002D72] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#00255A] transition-colors duration-300 text-sm">
                    View Details
                </button>
            </div>
        </div>
              ...
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {CAMELLIA_AGENTS_LIST.map(agent => (
                        <AgentTile key={agent.id} agent={agent} onDetailsClick={setSelectedAgent} />
                    ))}
                </div>
              </div>
              ...
              <div className="mt-12 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex flex-col sm:flex-row items-center gap-4">
                    <a
                        ...
                        className="inline-flex items-center bg-slate-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-slate-600 transition-colors duration-300 shadow-md text-center"
                    >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center bg-[#ED1C24] text-white font-bold py-3 px-8 rounded-lg hover:bg-[#c91a20] transition-colors duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 text-center"
                >
                  Demo
                </a>
              </div>
